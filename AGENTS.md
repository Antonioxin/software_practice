...
- `project-implementation/apps/api/`: the shared Java 21 + Spring Boot application. Source is in `src/main/java/`, backend tests are in `src/test/java/`, and Flyway migrations are in `src/main/resources/db/migration/`. The application root package is `wemove`, with `WemoveApplication` at that root. Business packages such as `wemove.identity`, `wemove.catalog`, and future `wemove.commerce` are siblings. Shared HTTP errors, transactions, idempotency persistence and request filters belong to `wemove.platform`; global configuration belongs to `wemove.config`. Never nest another business module under `identity`.
