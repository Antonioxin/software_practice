...
jar_path="$project_dir/apps/api/target/wemove-api-1.0.0.jar"
...
if [[ ! -f "$jar_path" ]]; then
  echo "未找到后端 JAR，请先执行：cd project-implementation/apps/api && mvn package" >&2
  exit 1
fi

run_dir="$(mktemp -d /tmp/wemove-identity-smoke.XXXXXX)"
