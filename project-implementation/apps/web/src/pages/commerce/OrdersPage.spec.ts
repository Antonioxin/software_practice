...
describe('订单列表中的商品摘要', () => {
  it('直接使用订单快照名称、数量和SKU图片，不额外读取商品或订单详情', async () => {
    const order = summary({ items: [
      { productId: 'saved-1', name: '下单时的平衡石名称', sku: 'WM-BALANCE-STONES', quantity: 2 },
    ...
    expect(row.text()).toContain('下单时的平衡石名称')
    expect(row.text()).toContain('数量 2')
  ...
  it('未知SKU仍显示历史名称和数量，不借用其他商品图片', async () => {
    ...
    expect(page.get('.order-product').text()).toContain('数量 1')
