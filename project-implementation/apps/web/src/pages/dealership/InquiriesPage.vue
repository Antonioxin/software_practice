...
import { formatProductName } from '../../features/catalog/presentation'
...
<template>
  <SiteShell :title="admin ? '询价管理' : '我的询价'" :admin="admin" :eyebrow="admin ? 'OPERATIONS / INQUIRIES' : 'DEALER / INQUIRIES'">
    <section class="paper-section">
      <div class="dealer-toolbar"><div class="dealer-filter-select"><SketchIcon name="filter" :size="24" /><select v-model="status" @change="load"><option value="">全部状态</option><option v-for="(label, value) in inquiryLabels" :key="value" :value="value">{{ label }}</option></select></div><RouterLink v-if="!admin" class="primary-button" to="/dealer/catalog">新建询价</RouterLink></div>
      ...
      <div v-else class="dealer-list">
        <article v-for="inquiry in inquiries" :key="inquiry.id">
          <div><p>{{ inquiry.inquiryNumber }}</p><h2>{{ inquiry.items.map((item) => formatProductName(item.name)).join('、') }}</h2><span>{{ new Date(inquiry.createdAt).toLocaleString('zh-CN') }} · {{ inquiry.items.length }} 项商品</span></div>
