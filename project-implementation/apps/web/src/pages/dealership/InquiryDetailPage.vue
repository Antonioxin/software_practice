    ...
    <template v-else-if="inquiry">
      ...
      <section v-if="inquiry.status !== 'CLOSED'" class="paper-section dealer-actions-panel">
        <template v-if="admin">
          ...
          <div class="dealer-reply-form"><label>公开回复<textarea v-model="reply" maxlength="2000"></textarea></label><div v-for="item in inquiry.items" :key="item.id" class="dealer-reply-line"><strong>{{ formatProductName(item.name) }}</strong><label>回复参考价（分）<input v-model.number="replyPrices[item.id]" type="number" min="1" max="99999999" /></label><label>预计交期<input v-model="leadTimes[item.id]" maxlength="500" /></label></div><button class="primary-button" type="button" :disabled="busy || !reply.trim()" @click="command('replies')">保存公开回复</button></div>
        </template>
      ...
      <section class="paper-section"><h2 class="wm-icon-label"><SketchIcon name="history" :size="28" /> 状态历史</h2><ol class="dealer-history"><li v-for="item in inquiry.history" :key="`${item.createdAt}-${item.action}`"><strong>{{ inquiryLabels[item.toStatus] }}</strong><span>{{ new Date(item.createdAt).toLocaleString('zh-CN') }}{{ item.reason ? ` · ${item.reason}` : '' }}</span></li></ol></section>
    </template>
  </SiteShell>
</template>
