...
商品列表使用的拍立得边框见 [frames/polaroid-product.png](frames/polaroid-product.png)，直接复制自[拍立得商品展示框.png](../../../../前端设计视觉参考/视觉组件/拍立得商品展示框.png)，尺寸为 1149 × 1369，大小 526,233 字节。保留原始透明通道和纸面纹理，未裁切、重绘或压缩；不要替换为深浅背景对照预览图。商品展示内容位于透明窗口下层，手写名称位于下侧宽边框，三层按同一画布比例定位。边框文件随前端交付，不依赖参考素材所在的本地路径。

## 商品展示图

商品列表另使用 [products/](products/) 中的六张 AI 商品概念图，分别对应平衡石、彩虹拱桥、环环投掷、伙伴协作板、森林探索套装和轻盈跳绳。图片通过内置 imagegen 按商品名称及简介生成，参考用户提供截图的浅灰棚拍背景与柔光，不是实际商品照片。每张保留生成工具输出的 1254 × 1254 RGB PNG，六张合计 10,818,846 字节；与生成原件逐字节一致。完整提示词、SKU 对应及验证见[商品展示图生成记录](../../../../docs/modules/content/商品展示图生成记录.md)。

运行地址均为 `/assets/products/文件名.png`，不依赖个人缓存路径。商品页前两张立即加载，后续图片延迟加载；未知 SKU 或单张图片加载失败时回退手绘占位。原始 PNG 会随 Vite 的 public 资源复制到构建目录，其他成员不需要重新生成图片。

### 商品玩法配图

详情页的 [products/guides/](products/guides/) 保存六款商品的玩法配图，采用“实物质感商品＋简笔卡通人物”的混合表现：商品保留摄影式木纹、织物和光影，人物使用平面手绘线条。由内置 imagegen 参考上方商品图生成，作为玩法示意；原有商品图不变。运行文件独立保存在前端 public 目录，完整提示词和对应关系见[商品玩法配图生成记录](../../../../docs/modules/content/商品玩法配图生成记录.md)。

每张图搭配网页中的三个文字步骤，图片采用完整画布展示与延迟加载。图解加载失败时保留文字步骤；未配置图解的商品继续呈现后端使用说明。
