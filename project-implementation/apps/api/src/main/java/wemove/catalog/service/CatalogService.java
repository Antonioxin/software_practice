...
public class CatalogService {
    ...
    public Page<CatalogDtos.ProductCard> publicProducts(
            String keyword,
            UUID categoryId,
            Integer age,
            PlayType playType,
        ...
        Specification<ProductEntity> spec =
                (root, query, cb) -> {
                    ...
                    if (scene != null) {
                        if (scene == ProductScene.BOTH)
                            filters.add(cb.equal(root.get("scene"), ProductScene.BOTH));
    ...
    public CatalogDtos.PublicProductDetail publicDetail(UUID id) {
        ...
        if (!listed) {
            return new CatalogDtos.PublicProductDetail(
                    product.getId(),
                    product.getSku(),
                    product.getName(),
                    product.getSummary(),
                    ...
                    List.of(),
                    valueOrZero(product.getRetailUnitPriceFen()),
                    "CNY",
                    stock > 0,
        ...
        return new CatalogDtos.PublicProductDetail(
                ...
                product.getPackageContents(),
                product.getInstructions(),
                product.getSafetyNotes(),
                product.getMainImageId(),
    ...
    public CatalogDtos.ProductOptions productOptions() {
        return new CatalogDtos.ProductOptions(
                List.of(
                        option(PlayType.BALANCE, "平衡能力"),
                        option(PlayType.COORDINATION, "协调训练"),
                        option(PlayType.THROWING, "投掷与瞄准"),
                        option(PlayType.TEAM_PLAY, "团队游戏"),
                        option(PlayType.OUTDOOR_EXPLORATION, "户外探索")),
                List.of(
                        option(ProductScene.INDOOR, "室内"),
                        option(ProductScene.OUTDOOR, "户外"),
                        option(ProductScene.BOTH, "室内与户外")));
    }

    @Transactional(readOnly = true)
    public Page<CatalogDtos.AdminProductView> adminProducts(
            String keyword, ProductStatus status, UUID categoryId, Pageable pageable) {
        String term = CatalogRules.trimToNull(keyword);
        Specification<ProductEntity> spec =
                (root, query, cb) -> {
                    List<Predicate> filters = new ArrayList<>();
    ...
    @Transactional(readOnly = true)
    public CatalogDtos.AdminProductView adminProduct(UUID id) {
        return adminView(requireProduct(id));
    }

    public CommandResult<CatalogDtos.AdminProductView> createProduct(
            UUID actorId, UUID key, CatalogDtos.CreateProductRequest request) {
        var execution =
                executor.execute(
                        actorId,
                        ...
                        () -> {
                            ...
                            if (sku != null && products.existsBySku(sku))
                                unique("sku", "SKU 已存在，请使用其他编号。");
                            ...
                            InventoryBalanceEntity balance =
                                    balances.save(
                                            new InventoryBalanceEntity(
                                                    product.getId(), initialStock, now));
                            movements.save(
    ...
    @Transactional
    public CatalogDtos.AdminProductView updateProduct(
            UUID actorId, UUID id, CatalogDtos.UpdateProductRequest request) {
        CatalogRules.validateDraft(request);
        ...
        CategoryEntity category =
                request.categoryId() == null ? null : requireCategory(request.categoryId());
        Instant now = clock.instant();
        apply(product, request, category, now);
        ...
        audit.append(
                new AuditPort.AuditEvent(
                        actorId, "PRODUCT_UPDATED", "PRODUCT", id, "SUCCESS", null, now));
        return adminView(product);
    }

    public CommandResult<CatalogDtos.AdminProductView> changePublication(
            UUID actorId, UUID id, UUID key, CatalogDtos.VersionCommand request, boolean publish) {
        var execution =
                executor.execute(
                        actorId,
                        publish ? "catalog.publishProduct" : "catalog.unpublishProduct",
                        key,
                        "/api/v1/admin/products/" + id + (publish ? "/publish" : "/unpublish"),
                        request,
                        CatalogDtos.AdminProductView.class,
                        200,
                        hash(request),
                        () -> {
                            ProductEntity product =
                                    products.findForUpdateById(id)
                                            .orElseThrow(this::notFoundException);
                            if (product.getVersion() != request.expectedVersion())
                                versionConflict();
                            if (publish && product.getStatus() == ProductStatus.PUBLISHED
                                    || !publish && product.getStatus() != ProductStatus.PUBLISHED) {
                                throw new ApiException(
                                        HttpStatus.CONFLICT, "STATE_CONFLICT", "商品已处于目标状态。");
                            }
                            Instant now = clock.instant();
                            if (publish) {
                            ...
                            audit.append(
                                    new AuditPort.AuditEvent(
                                            actorId, action, "PRODUCT", id, "SUCCESS", null, now));
    ...
    public CommandResult<CatalogDtos.CategoryView> createCategory(
            UUID actorId, UUID key, CatalogDtos.CategoryCreateRequest request) {
        var execution =
                executor.execute(
                        actorId,
                        ...
                        () -> {
                            ...
                            if (categories.existsByNameNormalized(normalized))
                                unique("name", "分类名称已存在。");
                            Instant now = clock.instant();
    ...
    @Transactional
    public CatalogDtos.CategoryView updateCategory(
            UUID actorId, UUID id, CatalogDtos.CategoryUpdateRequest request) {
        validateCategory(request.name(), request.description());
        ...
        entity.update(
                name,
                normalized,
                CatalogRules.trimToNull(request.description()),
                request.sortOrder(),
                request.enabled(),
                clock.instant());
        audit.append(
    ...
    private CatalogDtos.ProductCard card(ProductEntity product) {
        ...
        return new CatalogDtos.ProductCard(
                product.getId(),
                product.getSku(),
                product.getName(),
                product.getSummary(),
    ...
    private CatalogDtos.AdminProductView adminView(ProductEntity product) {
        return adminView(product, balance(product.getId()));
    }

    private CatalogDtos.AdminProductView adminView(
            ProductEntity product, InventoryBalanceEntity balance) {
    ...
    private void apply(
            ...
            Instant now) {
        product.apply(
                CatalogRules.trimToNull(request.sku()),
                CatalogRules.trimToNull(request.name()),
                category,
                CatalogRules.trimToNull(request.summary()),
                CatalogRules.trimToNull(request.description()),
                request.ageMin(),
                request.ageMax(),
                request.playType(),
                request.scene(),
                CatalogRules.trimToNull(request.material()),
                CatalogRules.trimToNull(request.dimensions()),
                CatalogRules.trimToNull(request.packageContents()),
                CatalogRules.trimToNull(request.instructions()),
                CatalogRules.trimToNull(request.safetyNotes()),
                CatalogRules.trimToNull(request.mainImageId()),
                imageIdsText(request.imageIds()),
                request.retailUnitPriceFen(),
                Boolean.TRUE.equals(request.dealerEnabled()),
                request.dealerReferenceUnitPriceFen(),
                request.minInquiryQuantity(),
                CatalogRules.trimToNull(request.leadTimeText()),
    ...
    private void ensurePublishable(ProductEntity product, InventoryBalanceEntity balance) {
        CatalogRules.requirePublishable(
                product.getSku(),
                product.getName(),
                product.getCategory(),
                product.getSummary(),
        ...
        if (balance.getQuantity() < 0)
            throw new IllegalStateException("negative stock invariant violated");
    }

    private InventoryBalanceEntity balance(UUID productId) {
        return balances.findById(productId)
                .orElseThrow(
                        () ->
                                new ApiException(
                ...
                .orElseThrow(
                        () ->
                                new ApiException(
                                        ...
                                        "分类不存在。",
                                        List.of(
                                                new ApiException.FieldViolation(
                                                        "categoryId",
                                                        "RESOURCE_NOT_FOUND",
                                                        "请选择有效分类。"))));
    }
    ...
    private CatalogDtos.CategoryView category(CategoryEntity entity) {
        ...
        return new CatalogDtos.CategoryView(
                entity.getId(),
                entity.getName(),
    ...
    private CatalogDtos.Option option(Enum<?> value, String label) {
        return new CatalogDtos.Option(value.name(), label);
    }
    ...
    private List<String> imageIds(String value) {
    ...
    private void validateCategory(String name, String description) {
        ...
        if (description != null
                && description.strip().codePointCount(0, description.strip().length()) > 500) {
    ...
    private void versionConflict() {
        throw new ApiException(HttpStatus.CONFLICT, "VERSION_CONFLICT", "记录已更新，请刷新后重试。");
    }

    private void inUse() {
        throw new ApiException(HttpStatus.CONFLICT, "RESOURCE_IN_USE", "分类正在被商品引用，请先迁移商品。");
    }
