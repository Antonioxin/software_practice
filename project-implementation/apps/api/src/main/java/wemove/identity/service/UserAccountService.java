...
public class UserAccountService {
    ...
    public Dtos.Actor register(Dtos.RegisterRequest request) {
        ...
        consents.save(
                new UserConsentEntity(
                        user.getId(), request.termsVersion(), request.privacyVersion(), now));
        return actor(user);
    }
    ...
    public Dtos.Actor updateProfile(UUID id, Dtos.ProfileRequest request) {
        ...
        if (user.getBaseRole() != BaseRole.USER) {
            throw new ApiException(
                    HttpStatus.FORBIDDEN, "ADMIN_PROFILE_READ_ONLY", "管理员资料不通过个人中心修改。");
        }
        String nickname = request.nickname().strip();
        validateCodePoints("nickname", nickname, 2, 30, "昵称需为 2—30 个字符。");
    ...
    @Transactional(readOnly = true)
    public Page<Dtos.UserSummary> list(
            String email, String nickname, BaseRole role, AccountStatus status, Pageable pageable) {
        Specification<UserEntity> spec =
                (root, query, cb) -> {
                    ...
                    if (email != null && !email.isBlank())
                        predicates.add(
                                cb.like(
                                        root.get("emailNormalized"),
                                        "%" + normalizeEmail(email) + "%"));
                    if (nickname != null && !nickname.isBlank())
                        predicates.add(cb.like(root.get("nickname"), "%" + nickname.strip() + "%"));
    ...
    public Dtos.UserDetail detail(UUID id) {
        UserEntity user = requireUser(id);
        List<Dtos.StatusHistory> items =
                histories.findByUserIdOrderByCreatedAtDesc(id).stream()
                        .map(
                                h ->
                                        new Dtos.StatusHistory(
                                                h.getAction(),
                                                h.getPreviousStatus(),
                                                h.getNewStatus(),
                                                h.getReason(),
    ...
    public CommandResult changeStatus(
            UUID actorId, UUID targetId, String action, UUID key, Dtos.AccountCommand request) {
        var execution =
                executor.execute(
                        actorId,
                        action.equals("DISABLE") ? "disableUser" : "restoreUser",
                        key,
                        "/api/v1/admin/users/" + targetId + "/" + action.toLowerCase(Locale.ROOT),
                        request,
                        Dtos.UserSummary.class,
                        200,
                        sha256(
                                targetId
                                        + "|"
                        ...
                        () -> {
                            UserEntity target =
                                    users.findForUpdateById(targetId)
                                            .orElseThrow(
                                                    () ->
                                                            new ApiException(
                            ...
                            if (target.getVersion() != request.expectedVersion()) {
                                throw new ApiException(
                                        HttpStatus.CONFLICT, "VERSION_CONFLICT", "账户已更新，请刷新后重试。");
                            }
                            AccountStatus next =
                            ...
                            if (target.getAccountStatus() == next) {
                                throw new ApiException(
                                        HttpStatus.CONFLICT, "STATE_CONFLICT", "账户已处于目标状态。");
                            }
                            Instant now = clock.instant();
                            ...
                            histories.save(
                                    new AccountStatusHistoryEntity(
                                            ...
                                            request.reason().strip(),
                                            now));
                            if (next == AccountStatus.DISABLED)
                                revokeSessions(target.getEmailNormalized());
                            audit.append(
    ...
    private Dtos.UserSummary summary(UserEntity user) {
        return new Dtos.UserSummary(
                ...
                user.getBaseRole().name(),
                user.getAccountStatus().name(),
                dealerIdentity.derivedIdentity(user.getId()),
                user.getVersion(),
                user.getCreatedAt(),
                user.getUpdatedAt());
    }
    ...
    private UserEntity requireUser(UUID id) {
        return users.findById(id)
                .orElseThrow(
                        () ->
                                new ApiException(
                                        HttpStatus.NOT_FOUND, "RESOURCE_NOT_FOUND", "记录不存在或不可访问。"));
    }
    ...
    private String normalizePhone(String phone) {
        ...
        if (!normalized.matches("^\\+?\\d{6,20}$"))
            throw field("phone", "INVALID_PHONE", "请输入有效联系电话，可包含国家区号。");
        return normalized;
    }
    ...
    private void validatePassword(String password, String confirmation) {
        ...
        if (!password.equals(confirmation))
            throw field("confirmPassword", "PASSWORD_MISMATCH", "两次输入的密码不一致。");
    }
