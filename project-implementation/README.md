...
| [apps/api/src/main/resources/db/migration/](apps/api/src/main/resources/db/migration/) | Flyway 数据库迁移 | V1 为身份底座，V2 为商品库存，V3 撤销旧包名登录会话；新变更追加有序迁移 |
...
| [项目进度表](../文档撰写/项目进度表/) | 项目进度与流程展示资料，含甘特图 PDF 和可编辑的 Excel 文件 | 随仓库共享，调整计划时同步维护展示稿与可编辑文件 |
| [LaTeX 需求文档](../文档撰写/项目需求文档/编译说明.md) | 正式 PDF、LaTeX 源码与编译入口 | 源码、必要资源与正式 PDF 一并维护；压缩包和排版检查产物留在本地 |
...
Java 根包统一为 `wemove`，`identity`、`catalog` 及后续 `commerce` 等业务模块平级；公共设施放入 `platform`，全局配置放入 `config`。数据库迁移保留在 Flyway 默认扫描位置。包名升级会通过 V3 一次性撤销已有登录会话，用户需重新登录；业务数据保留。
...
| 后端业务代码 | `apps/api/src/main/java/wemove/<业务域>/` | 参照现有 `catalog/` 分为 `api`、`service`、`domain`、`repository`；跨模块端口按需放 `platform` |
...
4. 后端功能进入 `apps/api/src/main/java/wemove/commerce/`，对应契约进入 `contracts/openapi/commerce.yaml`；测试和模块文档按上表放置。
